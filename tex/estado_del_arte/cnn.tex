...
\begin{figure}[htbp]
    ...
    \caption{Esquema general de una red neuronal convolucional adaptada a la clasificación de intención de movimiento a partir de señales EEG (Elaboración propia).}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{img/Esquema_CNN_GeneralEEG.png}
    %\includegraphics[height=\dimexpr\pagegoal-\pagetotal-\baselineskip\relax,width=\textwidth,keepaspectratio]{img/diagrama_pipeline.drawio.png}
    \caption{Esquema general de una red neuronal convolucional para la tarea de clasificación binaria en EEG (Elaboración propia).}
\end{figure}


A pesar de las ventajas que aportan estas redes neuronales, uno de los principales desafíos en su aplicación es el diseño de la arquitectura, la selección de hiperparámetros y la necesidad de disponer de una cantidad suficiente de datos para entrenar modelos robustos. Para mitigar estas limitaciones, se han propuesto estrategias como el \textit{transfer learning}, la cual consiste en utilizar modelos previamente entrenados en tareas similares y adaptarlos a la tarea específica mediante un proceso de ajuste fino (\textit{fine-tuning}). El transfer learning se utiliza habitualmente en escenarios donde la cantidad de datos disponibles es limitada, ya que permite aprovechar el conocimiento adquirido por el modelo en un dominio diferente; o donde la arquitectura de la red es compleja y modificarla desde cero sería costoso en términos de tiempo y recursos computacionales \cite{sambyal_RecentDevelopments2023,pan_SurveyTransfer2010}.
...
Como se ha mencionado en el \hyperref[sec:metodos_tradicionales]{punto anterior}, los sistemas BCI basados en EEG dependían de técnicas de extracción manual de características combinadas con clasificadores convencionales. Sin embargo, la aparición de arquitecturas de redes neuronales profundas específicas para EEG, como DeepConvNet, ShallowConvNet y EEGNet, supuso un cambio de paradigma al integrar de forma conjunta las etapas de extracción de características y clasificación dentro de una misma red neuronal \cite{schirrmeister_DeepLearning2017,kollod_DeepComparisons2023,lawhern_EEGNetCompact2018}.
