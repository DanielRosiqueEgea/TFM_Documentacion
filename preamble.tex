...
% % Se añade una profundidad adicional a los títulos (profundidad 4):
% \usepackage{titlesec}
% \setcounter{secnumdepth}{5} % Se fija en 4 la profundidad de numeración de títulos.
% \setcounter{tocdepth}{5} % Se fija en 4 la profundidad de títulos incluidos en el índice.
% % Se modifica el formato de \paragraph (título de profundidad 4) para adaptarlo al formato del resto de títulos:
% \titleformat{\paragraph}
% {\normalfont\normalsize\bfseries}{\theparagraph}{1em}{}
...
\fancyhead[RO]{Redes Neuronales Convolucionales Para el Análisis de Señales EEG} % Texto que se coloca en el encabezado de las páginas impares (O -> 'Odd', o impar) a la izquierda (R -> 'Odd')
...
\fancyfoot[RE]{Máster Universitario en Robótica} % Texto que se coloca en el pie de página de las páginas pares (E -> 'Even', o par) a la derecha (R -> 'Right')
\fancyfoot[LO]{Daniel Rosique Egea} % Texto que se coloca en el pie de página de las páginas impares (O -> 'Odd', o impar) a la izquierda (L -> 'Left')
...
\usepackage{tikz}
\usetikzlibrary{shadows, trees, shapes.geometric, arrows.meta, positioning, shapes.geometric}
\tikzset{
...
\lstdefinestyle{pythonstyle}{
  ...
  keywords=[3]{
      True, False, ttk, list, None, dict,
      zip, range, len, print, float, sum
    },
  keywordstyle={[4]\bfseries\ttfamily},
  keywords=[4]{_read_excel},
...
\lstdefinestyle{sqlstyle}{
  ...
  stringstyle=\color{codegreen},
  morekeywords={
      CREATE, REPLACE, VIEW, WITH,
      FILTER, STDDEV_SAMP, NULLIF
    }
}
\lstdefinestyle{mystyle}{
  ...
  % - Palabras clave:
  deletekeywords={print}, % Se elimina "print" del conjunto de palabras clave para osteriormente asignarle el color morado.
  keywordstyle={[2]\ttfamily\color{codeblue}},
  keywords=[2]{as}, % Se añaden las palabras clave de color azul.
  keywordstyle={[3]\ttfamily\color{codepurple}},
  keywords=[3]{True, False, ttk, list, None, dict, zip, range, len, print, float, um}, % Se añaden las palabras clave de color morado.
  keywordstyle={[4]\bfseries\ttfamily},
  ...
  % - Números de línea:
  numberstyle=\tiny\color{codegray}, % Tamaño de fuente y color de los números de ínea.
  numbers=left, % Se colocan los números de línea en el lado zquierdo.                 
  ...
  postbreak = \mbox{{\(\hookrightarrow\)}\space}, % Se añade una flecha al cambiar e línea.
